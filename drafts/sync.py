#! env variables `python`` and `git` are required.
...
if __name__ == '__main__':
    ...
    # install hook for sync *.md.html with *.md
    hook = ".git/hooks/pre-commit"
    if not os.path.exists(hook):
        with open(hook, 'w+') as f:
            f.write("#!/bin/bash\npython drafts/sync.py")

    # sync all changed *.md
    html_list = ""
    for file in os.popen("git diff-index --cached --name-only HEAD").read().splitlines():
        if file != "README.md" and file.endswith(".md") and os.path.exists(file):
            print(file)
